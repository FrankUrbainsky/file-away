...
function sssc_fileaway($atts){
	...
	if($directories):
		...
		foreach ($crumbs as $k => $crumb):
			...
			if ($basebase === $basecheck): $crumblink[$k] = ltrim(ssfa_replace_first($basebase, '', $crumblink[$k]), '%2C'); endif;
	...
	if ($type === 'table'):
		...
		$thefiles .= 
			...
			<th class='ssfa-sortname' title='Click to Sort'".$filenamesort.">$filename</th>";
	...
	if ($directories):
		foreach ( glob($dir."/*", GLOB_ONLYDIR) as $k=> $folder ):
			...
			$dlink = str_replace('/', '%2C', ltrim($dlink, '/'));
			$thefiles .= "<tr class='ssfa-drawers'><td data-value='00-$folder' class='ssfa-sorttype $style-first-column'><a href='?drawer=$dlink'><span data-ssfa-icon='=' style='font-size:20px; margin-left:3px;' class='$iconstyle $icocol' aria-hidden='true'></span><br>dir</a></td><td data-value='00-$folder' class='ssfa-sortname'><h4 style='text-transform:uppercase;'><a href='?drawer=$dlink'>$folder</a></h4></td>"; 			
...
function ssfa_fileaframe($atts){
	extract(shortcode_atts(array(
		...
		'mheight' => '0px', 'seamless' => 'seamless'
	), $atts));
	$seamless = ($seamless !== 'seamless' ? null : 'seamless');
	if($source)
	return "<iframe src='$source' width=$width height=$height scrolling=$scroll frameborder=$frame marginwidth=$mwidth marginheight=$mheight $seamless></iframe>";
}
