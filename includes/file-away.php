...
function sssc_fileaway($atts){
	...
	if($directories):
		...
		foreach ($crumbs as $k => $crumb):
			...
			$prettycrumb = ucwords(preg_replace('/(?<=\D)-(?=\D)/', ' ', $prettycrumb));
			$prettycrumb = ucwords(preg_replace('/(?<=\d)-(?=\D)/', ' ', $prettycrumb));
			$prettycrumb = ucwords(preg_replace('/(?<=\D)-(?=\d)/', ' ', $prettycrumb));
			if($crumb !== ''):
				...
				$thefiles .= '<a href="?drawer='."$crumblink[$k]".'">'."$prettycrumb".'</a>/';
			endif;
		endforeach;
		$thefiles .= "</div>";
	endif;
	...
	if ($manager): 
		$thefiles .= "<div id='ssfa-path-container' style='display:none; float:left;'>
						<div id='ssfa-directories-select-container' class='frm_form_field form-field frm_required_field frm_top_container frm_full'>
							<label for='ssfa-directories-select' class='frm_primary_label'>
