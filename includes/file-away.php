...
function sssc_fileaway($atts){
	...
	$path = null; $ssh = null; $sh = null; $td = null; $bb = null; $fafl = null; $faui = null; $faun = null; $faur = null; 
		...
		if(count($checksubdirs) > 0): $f = 0;
			foreach ( glob("$dir"."/*", GLOB_ONLYDIR) as $k=> $folder ):
				...
				if (! $direxcluded):			
					...
					$managedir = $manager ? "<a href='' id='rename-ssfa-dir-$uid-$f'>Rename</a><br><a href='' id='delete-ssfa-dir-$uid-$f'>Delete</a></td>" : ' &nbsp; '; 
					$renamedir = $manager ? '<input id="rename-ssfa-dir-'.$uid.'-'.$f.'" type="text" value="'.$folder.'" '.
	...
	$thumbnails = $thumbnails && $type === 'table' && extension_loaded('gd') && function_exists('gd_info') ? true : false;
	if($thumbnails): 
	...
	if(is_array($rawnames)):
		...
		foreach($rawnames as $k => $rawname):
			...
			if(is_file($dir.'/'.$file) && $thename !== ''):
				...
				if($getthumb): 	
					$thumbpath = (SSFA_ROOT === 'siteurl' ? stripslashes($dir) : 
						($ssfa_install ? ssfa_replace_first($ssfa_install, '', stripslashes($dir)) : stripslashes($dir)));
					$imgprop = getimagesize($ssfa_abspath.$thumbpath.'/'.$file);
					while($getthumb):
