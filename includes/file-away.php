...
function sssc_fileaway ( $atts ) {
	...
	if ( $type === 'table' ) {
		...
		$thefiles .= ( $customdata ? "<th class='ssfa-sortcustomdata' title='Click to Sort'".$customsort.">$customdata</th>" : null );
	...
	foreach ( $files as $file ) {
		...
		if ( is_file ( $dir.'/'.$file ) ) {
			...
			if ( $exclusions ) {			
				...
				if ( !$type || $type !== 'table' ) 				
					$thefiles .= 				
						"<a id='ssfa' class='$display$noicons$colors' href='$link' $linktype>						
						<div class='ssfa-listitem $ellipsis'><span class='ssfa-topline'>$icon $name $listfilesize</span> $datemodified</div>					
						</a>"; 				
				if ( $type === 'table' ) {
					$thefiles .= 
						"<tr><td class='ssfa-sorttype $style-first-column'><a href='$link' $linktype>$icon $ext</a></td>
						<td class='ssfa-sortname'><a href='$link' $linktype>$name</a></td>";
