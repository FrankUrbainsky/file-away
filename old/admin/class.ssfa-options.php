...
class File_Away_Options {
	...
	public function add_pages() {
		...
		if (is_plugin_active("wp-editor/wpeditor.php") && $_SERVER['QUERY_STRING'] == 'page=file-away') {
	...
	public function display_page() {
		...
		if ( $randysave === 4 ) { $saving = 'Gettin\' saved...'; $saved = 'Hallelujah, by and by.'; }
		if ( $randysave === 5 ) { $saving = 'Just a few more weary days and then...'; $saved = '...your settings will be saved.'; }
		
		echo '<div class="wrap">
				<img src="'.SSFA_IMAGES_URL.'fileaway_banner.png" style="width:500px;">';
		...
		<div id='ssfa-saving-img'><img src='".SSFA_IMAGES_URL."saving.gif'></div>		
		...
		if (!empty($ssfa_customcss))
			ssfa_create_css();
		elseif (empty($ssfa_customcss) && file_exists(css_path()))
			unlink(css_path());
	}
	...
	public function display_about_section() {
		include_once SSFA_ADMIN_RESOURCES.'about.php'; 				
	}
	...
	public function display_setting( $args = array() ) {
		...
		$chromefix = ( $is_chrome || $is_opera ? ' ssfa-abspath-chromefix' : null );
		...
		if ( $siteaddress !== '' && $siteaddress !== null && $siteaddress !== $wpaddress ) {
			$urldiff = str_replace($siteaddress, '', $wpaddress); 
			$ssfa_abspath = str_replace( $urldiff, '', ABSPATH ); }
		else $ssfa_abspath = ABSPATH;
		...
		switch ( $type ) {
			...
			case 'text':
			default:
		 		echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="fileaway_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $options[$id] ) . '" />' . $helplink;
		 		if ( $desc != '' )
		 			echo '<br /><div class="ssfa-description">' . $desc . '</div>';
				if ( $submit != null )
					echo '<br /><br /><br />'.$submit;
			...
			default:
				echo '<div id="ssfa-wrap-'.$id.'" class="ssfa-wrap-base"><span id="ssfa-abspath-'.$id.'" class="ssfa-abspath'.$chromefix.'">' . $abspath . '</span> <input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="fileaway_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $options[$id] ) . '" /></div> <input class="regular-text' . $field_class2 . '" type="text" id="' . $id2 . '" name="fileaway_options[' . $id2 . ']" placeholder="' . $std2 . '" value="' . esc_attr( $options[$id2] ) . '" />' . $helplink. '<br />
	...
	public function initialize_settings() {
		...
		foreach ( $this->settings as $id => $setting ) {
			if ( $setting['type'] != 'heading' )
				$default_settings[$id] = $setting['dflt'];
		}
	...
	public function styles() {
		wp_register_style( 'ssfa-options', SSFA_ADMIN_CSS_URL.'ssfa-options.css');
