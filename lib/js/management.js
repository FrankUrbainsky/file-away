...
jQuery(document).ready(function($)
{
	flightbox = function(url, uid, theme, icons, nolinks)
	{
		...
		if(!$('#ssfa-flightbox-shadow').length)
		{
			...
			$('body').append(FlightBox);
			theShadow.hide();
			FlightBox.hide();
		}
		else
		{
			var FlightBox = $('div#ssfa-flightbox');
			var FlightBoxCont = $('div.ssfa-flightbox-controls');
			var theShadow = $('div#ssfa-flightbox-shadow');	
		}
		...
		$next = $('div.ssfa-meta-container[data-uid="'+id[0]+'"] a[data-flightbox="'+(next)+'"]').get(0).attributes.onclick.value;		
		$prev = $('div.ssfa-meta-container[data-uid="'+id[0]+'"] a[data-flightbox="'+(prev)+'"]').get(0).attributes.onclick.value;
		$current = $('div.ssfa-meta-container[data-uid="'+id[0]+'"] a[data-flightbox="'+(id[1])+'"]').get(0).attributes.onclick.value;
		$nexturl = $('div.ssfa-meta-container[data-uid="'+id[0]+'"] a[data-flightbox="'+(next)+'"]').attr('href');
		$prevurl = $('div.ssfa-meta-container[data-uid="'+id[0]+'"] a[data-flightbox="'+(prev)+'"]').attr('href');
		$url = $('div.ssfa-meta-container[data-uid="'+id[0]+'"] a[data-flightbox="'+(id[1])+'"]').attr('href');
		...
		(
			...
			function(response)
			{
				$('div#ssfa-flightbox-inner').animate({opacity:'0'}, 500);
				$('div.ssfa-flightbox-controls a').animate({opacity:'0'}, 0);
				loading.remove();
				FlightBox.delay(500).animate({
					...
					$('div#ssfa-flightbox-inner').animate({opacity:'1'}, 500);
					$('div.ssfa-flightbox-controls span').css({opacity:'0.8'});
					if(response.iframe == 'true')
					{
						$('div#ssfa-flightbox-inner iframe').width(response.iwidth);
						$('div#ssfa-flightbox-inner iframe').height(response.iheight);	
					}
				});
			}
		)
	...
	Xflightbox = function()
	{
		$('div#ssfa-flightbox').fadeOut(500).queue(function(){$(this).remove();});
		$('div#ssfa-flightbox-shadow').fadeOut(500).queue(function(){$(this).remove();});
	}
});
jQuery(document).ready(function($)
{
