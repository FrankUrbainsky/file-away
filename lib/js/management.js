...
{
	flightbox = function(url, uid, theme, icons)
	{
...
jQuery(document).ready(function($)
{
	if($('div.ssfa_fileup_container').length)
	{
		...
		{
			return uid+'_'+name.replace(/[^a-z0-9\s]/gi, '_').replace(/[_\s]/g, '_');
		}	
						...
						selectedDisplayed += 
							'<tr id="ssfa_upfile_id_'+file_id+'" style="display: table-row;">'+
								...
								'<td id="ssfa_upfile_status_'+file_id+'" class="ssfa-sortstatus">'+
									'<a id="ssfa_remove_id_'+file_id+'" href="javascript:" onclick="fileupRemove(\''+file_id+'\',\''+aFile[uid][i].name+'\',\''+uid+'\',\''+i+'\');">'+
