...
$thefiles .= $username !== 'no' ? "<th class='ssfa-sortemail' $clicktosort>".__('Email', 'file-away')."</th>" : null;
...
if($records && count($records))
{
	...
	foreach($records as $record)
	{
		...
		$prettytime = date($datestring.' g:i A', strtotime($record['timestamp']));
		...
		$thefiles .= 
			"<tr id='ssfa-file-$uid-$count'>".
				...
				"<td id='file-ssfa-file-$uid-$count' class='ssfa-sortfile'><a href=\"".fileaway_utility::urlesc($url."/".$record['file'])."\" $download $datastat>".$record['file']."</a></td>".
