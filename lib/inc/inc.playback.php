...
if(!$skipthis && in_array($oext, $sources))
{
	...
	foreach($sources as $audioext)
	{
		...
		{ 
			...
			$players .= $redirect ? 
				'<a class="ssfa-audio-download'.$dlcolor.'" href="'.$this->op['redirect'].'" target="_blank">'
				: ($encryption 
					? '<a class="ssfa-audio-download '.$dlcolor.'" href="'.$encrypt->url($sourcefilepath.'.'.$audioext).'">' 
					: ($s2mem 
						? '<a class="ssfa-audio-download '.$dlcolor.'" href="'.$url.'/?s2member_file_download='.$s2sub.fileaway_utility::urlesc($rawname).'.'.$audioext.$s2skip.'">' 
						: '<a class="ssfa-audio-download '.$dlcolor.'" href="'.fileaway_utility::urlesc($sourcefileurl).'.'.$audioext.'" download="'.$rawname.'.'.$audioext.'">'
					)
				);
