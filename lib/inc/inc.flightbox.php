...
{ 
	...
	$boxquery = '?boxtype=image&g='.$guest.'&d='.$dimension.'&w='.$imgwdth.'&h='.$imghght.'&mw='.$maximgwidth.'&mh='.$maximgheight;
...
elseif(($flightbox == 'multi' || $flightbox == 'videos') && in_array(strtolower($extension), array('flv', 'mp4', 'm4v', 'webm', 'ogv', 'tube', 'vmeo')))
	$boxquery = '?boxtype=video&g='.$guest.'&e='.$extension.'&w='.$videowidth;
elseif(($flightbox == 'multi' || $flightbox == 'pdfs') && strtolower($extension) == 'pdf' && !$get->is_opera)
	$boxquery = '?boxtype=pdf&g='.$guest.'&r=wide';
if($boxquery) 
{
	$fulllink = $manager 
