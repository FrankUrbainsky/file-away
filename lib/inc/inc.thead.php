...
if($type === 'table')
{
	...
	if($directories) $filename = $drawerlabel ? $drawerlabel : _x('Drawer/File', 'File and Directory Name Column', 'file-away'); 
	else $filename = _x('File&nbsp;Name', 'File Name Column', 'file-away');
	if($manager)
	{	
	...
	$data_atts = $manager ? 'data-uid="'.$uid.'" data-pg="'.$GLOBALS['post']->ID.'" data-drw="'.$drawericon.'" data-cls="'.$theme.'" '.
		'data-basename="'.$basename.'" data-start="'.$start.'" data-dir="'.trim("$dir",'/').'" data-base="'.$basebase.'" data-basedir="'.$basedir.'"'.
		'data-fafl="'.$fafl.'" data-faui="'.$faui.'" data-faun="'.$faun.'" data-faur="'.$faur.'"' : null;
	...
	$thefiles .= 
		...
		"<table id='ssfa-table-$uid' data-filter='#filter-$uid' $disablesort $page class='footable ssfa-sortable $theme $textalign $bulkclass' $data_atts><thead><tr>".
	...
	if($customdata)
	{
		...
		foreach($customarray as $customdatum) if(preg_match('/[*]/', $customdatum)) $custom_sort = false;
	...
	$thefiles .= $mod !== 'no' 
		? "<th class='ssfa-sortdate' data-type='numeric' title=\""._x('Click to Sort', 'Column Sort Message', 'file-away')."\"".$modsort.">".
			_x('Date&nbsp;Modified', 'Date Modified Column', 'file-away')."</th>" 
		: null;
	$thefiles .= $size !== 'no' 
