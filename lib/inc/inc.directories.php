...
if($directories)
{
	...
	if($manager && $dirman)
	{
		...
		{ 
			$icell++; 
			if($icell < $ccell) $thefiles .= "<td class='$theme'> &nbsp; </td>"; 
			else $thefiles .= "<td id='manager-ssfa-create-dir-$uid' class='$theme'> &nbsp; </td>";
		}
	}
			...
			{			
				...
				foreach ($cells as $cell)
				{
					...
					$thefiles .= $icell < $ccell 
						? "<td class='$theme'> &nbsp; </td>" 
						: "<td id='manager-ssfa-dir-$uid-$f' class='$theme'>$managedir</td>";
				}
				$thefiles .= "</tr>";
			}
		}
