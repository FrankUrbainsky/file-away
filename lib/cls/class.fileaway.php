					...
					elseif($type == 'table')
					{
						...
						if($customdata && is_array($customarray))
						{
							...
							foreach($customarray as $k=> $customdatum)
							{
								if($customdatum !== ' ')
								{
									$value = !isset($customvalues[$k]) ? '' : ($prettify ? $customvalues[$k] : fileaway_utility::strtotitle(trim($customvalues[$k])));
									$custominput[$k] = $manager 
										? '<input class="'.$customdataclassname.'" id="customdata-'.$k.'-ssfa-file-'.$uid.'-'.$count.'" type="text" value="'.$value.'" '.
											'style="width:80%; height:26px; font-size:12px; text-align:center; display:none">' 
										: null;
