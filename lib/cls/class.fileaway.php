...
if(class_exists('fileaway_attributes') && !class_exists('fileaway'))
{
	class fileaway extends fileaway_attributes
	{
		...
		public function sc($atts)
		{
			...
			$thefiles .= "<div id='$name' class='ssfa-meta-container' data-uid='$uid' style='margin: 10px 0 0; $howshouldiputit'>";
			...
			{
				if(phpversion() < '5.4') array_multisort($rawnames, SORT_ASC, SORT_STRING, $links, $locs, $exts, $fulls, $dirs, $dynamics); 
				else array_multisort($rawnames, SORT_ASC, SORT_NATURAL, $links, $locs, $exts, $fulls, $dirs, $dynamics); 
				...
				foreach($rawnames as $k => $rawname)
				{
					...
					if(!$manager && basename($fulls[$k]) === 'fileaway-url-parser.csv') continue;
					$link = $links[$k];
					$loc = $locs[$k]; 
					$ext = $exts[$k]; 
					$oext = $ext; 
					...
					if(!$dynamiclink && (!is_file($dir.'/'.$file) || $thename == '')) continue;
					...
					$linktype = $dynamiclink ? 'target="_blank"' : $linktype;
					$audiocorrect = null;
