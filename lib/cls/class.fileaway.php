...
if(class_exists('fileaway_attributes') && !class_exists('fileaway'))
{
	class fileaway extends fileaway_attributes
	{
		...
		public function sc($atts)
		{
			...
			{ 
				$created_dir = mkdir($rootpath.$dir, 0775, true);
				if($created_dir) fileaway_utility::index($rootpath.$dir);
			}
			...
			if(is_array($rawnames))
			{
				...
				foreach($rawnames as $k => $rawname)
				{
					...
					{
						if($encryption) $link = $encrypt->url($rootpath.$dir.'/'.$file);	
						else $link = $unicode ? fileaway_utility::unicode(utf8_encode($link)) : $link;
						...
						if(!$type || $type != 'table')
							$thefiles .= 
								"$nolinkslist<div class='ssfa-listitem $ellipsis'>".
									"<span class='ssfa-topline'>$icon $thename $listfilesize</span> $datemodified".
								"</div></a>"; 				
						elseif($type == 'table')
						{
							...
							if($customdata)
							{
								...
								{
									if($customdatum !== null && $customdatum !== '')
									{
			...
			if($bulkdownload) include fileaway_dir.'/lib/inc/inc.bulk-download-content.php';
			$thefiles .= "</div></div>";
			if($logged_in && $private_content && $count > 0) return $thefiles; 	
