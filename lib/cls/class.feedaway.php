					...
					if($this->options['recursivefeeds'] == 'true')
					{
						...
						if(is_array($initdirs) && count($initdirs) > 0)
						{
							foreach($initdirs as $subdir)
							{
								...
								$subxml[] = 
										"<item>\n".
											"<title>".basename($subdir)."</title>\n".
											"<link>".fileaway_utility::replacefirst($subfeed, $rootpath, rtrim($this->options['baseurl'], '/').'/')."</link>\n".
											"<description>RSS Feed</description>\n".
											"<pubDate>".date($datestring, filemtime($subdir))."</pubDate>\n".
										"</item>\n";
							}
						}
					}
