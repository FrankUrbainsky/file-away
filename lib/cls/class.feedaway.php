...
if(!class_exists('feedaway'))
{
	class feedaway
	{
		...
		{
			$schedules['fifteenminutes'] = array('interval' => 900, 'display' => __('Every 15 Minutes'));
			$schedules['thirtyminutes'] = array('interval' => 1800, 'display' => __('Every 30 Minutes'));
			$schedules['fortyfiveminutes'] = array('interval' => 2700, 'display' => __('Every 45 Minutes'));
			$schedules['sixhours'] = array('interval' => 21600, 'display' => __('Every 6 Hours'));
			...
			foreach($basedirs as $basedir)
			{
				...
				foreach($dirs as $k => $dir)
				{
					...
					if(is_array($initfiles))
					{
						...
						{	
							if(fileaway_utility::startswith(basename($file), $startswith))
							{
								unset($initfiles[$i]);
								continue;
							}
							if(fileaway_utility::endswith(strtolower(basename($file)), $endswith))
							{
								unset($initfiles[$i]);
								continue;
							}
							...
							{
								if(strpos(basename($file), $str) !== false)
								{
									unset($initfiles[$i]);
									continue 2;
								}
							...
							$files['mime'][] = $mime;
							$files['datemodified'][] = filemtime($file);
						...
						$description = fileaway_utility::replacefirst($dir, $rootpath, rtrim($this->options['baseurl'], '/').'/');
						...
						if(isset($files['datemodified']) && count($files['datemodified']) > 0)
						{
							foreach($files['datemodified'] as $k => $file)
							{
								...
								$thename = preg_replace('/(?<=\d)-(?=\D)/', ' ', "$thename"); 
								$thename = preg_replace('/(?<=\D)-(?=\d)/', ' ', "$thename"); 
								$filedescription = $customvalue 
									? $customvalue.' ('.$files['extension'][$k]." file. Size: ".fileaway_utility::formatBytes($files['filesize'][$k]).')' 
									: $files['extension'][$k]." file. Size: ".fileaway_utility::formatBytes($files['filesize'][$k]);
								$xml .= 
									"<item>\n".
										"<title>".$thename."</title>\n".
										"<link>".$files['url'][$k]."</link>\n".
										"<description>".$filedescription."</description>\n".
										"<pubDate>".date($datestring, $file)."</pubDate>\n";
