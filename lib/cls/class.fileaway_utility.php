...
if(!class_exists('fileaway_utility'))
{
	class fileaway_utility
	{
		...
		public static function strtotitle($title)
		{
			...
			$words = explode(' ', $title); 
			if(is_array($words)) foreach($words as $key => $word) 
				if($key == 0 or !in_array($word, $excludearray)) $words[$key] = ucwords($word);
		...
		public static function sentencecase($string)
		{
			...
			$new_string = ''; 
			if(is_array($sentences)) foreach($sentences as $key => $sentence) 
