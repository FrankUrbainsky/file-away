...
if(!class_exists('fileaway_utility'))
{
	class fileaway_utility
	{
		...
		{
			return substr($source, -strlen($suffix)) == $suffix;
		}
		...
		public static function urls()
		{
			...
			$urls = array(
				$home1 => str_replace(array('http://', 'https://', 'www.'), '', $home1),
				$site1 => str_replace(array('http://', 'https://', 'www.'), '', $site1), 
				$url => str_replace(array('http://', 'https://', 'www.'), '', $url),
				$wpurl => str_replace(array('http://', 'https://', 'www.'), '', $wpurl),
				$site => str_replace(array('http://', 'https://', 'www.'), '', $site),
				$home => str_replace(array('http://', 'https://', 'www.'), '', $home),
				$network => str_replace(array('http://', 'https://', 'www.'), '', $network),
			);
		...
		public static function recursefiles($directory, $onlydirs, $excludedirs)
		{
			...
			{ 
				if($excludedirs) foreach($excludedirs as $exclude) if(self::endswith("$directory", "$exclude")) continue 2;
				$files_array = glob("{$directory}/*.*");
				$files_array = is_array($files_array) ? $files_array : array();
				foreach($files_array as $file) if(is_file($file)) $files[] = $file; 
			}
			return $files;
		}
		...
		public static function recursivedirs($directory)
		{
			...
			{ 
				...
				foreach($dir_array as $dir) if(is_dir($dir)) $dirs[] = $dir;
			}
			return $dirs;
		}
			...
			foreach($roles->roles as $role)
			{
				if(isset($role['capabilities']) && is_array($role['capabilities']))
					foreach($role['capabilities'] as $cap => $bool) 
						if(strpos($cap, 'level_') === false) $b[$cap] = $cap;
			}
		...
		public static function video($attr)
		{
			...
			foreach(array('poster', 'loop', 'autoplay', 'preload') as $a)
				if(empty($html_atts[$a])) 
					unset($html_atts[$a]);
			$attr_strings = array();
			foreach($html_atts as $k => $v) $attr_strings[] = $k.'="'.esc_attr($v).'"';
			$html = '';
