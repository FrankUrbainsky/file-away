...
if(class_exists('fileaway_attributes') && !class_exists('stataway'))
{
	class stataway extends fileaway_attributes
	{
		...
		public function userstats($atts)
		{
			...
			{
				$items[$i] = '<li>'.basename($record['file']);
				if($timestamp == 'yes') $items[$i] .= ' <span style="display:block;" class="'.$class.'timestamp">'.date($datestring.' g:i A', strtotime($record['timestamp'])).'</span>';
				$items[$i] .= '</li>';
			}
