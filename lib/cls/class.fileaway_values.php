...
if(class_exists('fileaway_attributes') && !class_exists('fileaway_values'))
{
	class fileaway_values extends fileaway_attributes
	{
		...
		public function sc($atts)
		{
			...
			extract($this->correct(wp_parse_args($atts, $this->fileaway_values), $this->shortcodes['fileaway_values'], 'fileaway_values'));
				...
				if($showoptions)
				{
					...
					if(is_array($files))
					{
						foreach($files as $key => $file)
						{
							...
							{
								$dir = $recursive ? str_replace(basename($file), '', $file) : $dir;
								$dir = rtrim($dir, '/');
							}
							$is_selected = $file === $dir.'/'.$filename ? 'selected' : null;
							$link = fileaway_utility::querystring(get_permalink(), $_SERVER['QUERY_STRING'], array('fa_csv' => base64_encode(basename($file)), 'fa_index' => $key));
							$option_display = $recursive ? ($install ? fileaway_utility::replacefirst($file, $install, '') : $file) : preg_replace('/.csv$/i', '', basename($file));
			...
			$thefiles .= $clearfix.
				"<div id='$name' class='ssfa-meta-container ssfa-fileaway-values $mobileclass' data-editor='$edit' data-uid='$uid' style='opacity:0; margin: 10px 0 20px; $howshouldiputit'>";
			if($filename && is_file($rootpath.$dir.'/'.$filename))
			{
				...
				$thefiles .= 
					...
					"<table id='ssfa-table-$uid' class='ssfa-values-table footable ssfa-sortable $theme $textalign' data-filter='#filter-$uid' data-uid='$uid' ".
						"data-theme=\"$theme\" data-src=\"".base64_encode($dir.'/'.$filename)."\" data-filename=\"".basename($filename)."\" ".
