		...
		public function manager()
		{
			...
			elseif($action === 'rename')
			{
				...
				if(!$metadata)
				{
					...
					if($customdata !== '') $customdata = " [".trim(ltrim(rtrim("$customdata", "]"), " ["), " ")."]";
				...
				else
				{
					$customdata = stripslashes($_POST['customdata']);		
					$customdata = rtrim("$customdata", ',');
					if($customdata !== '') $customdata = explode(',', $customdata);
					...
					if(is_file($newfile) && $meta_table_exists)				
						fileaway_utility::updatemetadata(
							...
							fileaway_utility::replacefirst($oldfile, $rootpath, ''), 
							fileaway_utility::replacefirst($newfile, $rootpath, ''));
					if($customdata !== '') $customdata = " [".implode(',', $customdata)."]";					
				}
				$response = array(
					"errors" => $errors, 
					"download" => $download, 
			...
			elseif($action == 'makecsv')
			{
				...
				{
					if(strpos($filename, '/') !== false && !is_dir($rootpath.$dir.'/'.dirname($filename)))
						mkdir($rootpath.$dir.'/'.dirname($filename), 0775, true);
					...
					if(is_file($rootpath.$dir.'/'.$filename))
					{
						...
						$link = fileaway_utility::querystring(get_permalink($_POST['pg']), $querystring, array('fa_csv' => base64_encode(basename($filename)), 'fa_index' => $file_index));
