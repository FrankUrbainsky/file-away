	...
	class fileaway_management
	{
		...
		public function manager()
		{
			...
			elseif($action === 'rename')
			{
				...
				$oldnameencode = fileaway_utility::unicode($oldname);
				$newurl = str_replace("$pp/$oldnameencode.$ext", "", str_replace('%23', '#', "$url"));
				$newurl = str_replace('#', '%23', "$newurl$pp/".trim("$rawname", ' ')."$customdata.$ext");
				$newurl = fileaway_utility::unicode($newurl);
				$newoldname = trim("$rawname", ' ')."$customdata";
				$download = trim("$rawname", ' ')."$customdata.$ext";		
				$oldfile = iconv("UTF-8", "CP1252", $oldfile);
				$newfile = iconv("UTF-8", "CP1252", $newfile);
