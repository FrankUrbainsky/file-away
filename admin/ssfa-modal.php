...
<div class="ssfamodal_option" style="display:inline-block;">
<input type="text" placeholder="Source URL" id="ssfamodal-fi-src" class="ssfamodal_isrc" name="src" value="" />
<?php ssfa_helplink('isrc') ?>
</div>
...
<option value="godcomplex">Heavens to Murgatroid</option>
</select>
...
<td  style="vertical-align: top;"> <div class="ssfamodal_option" style="display:inline-block;">
...
<?php ssfa_helplink('role_override') ?></div></td>
...
jQuery(document).ready(function($) {
	...
	$('select#ssfamodal-ft-manager').on('change', function(){
		$manmode = this.value;
		if($manmode !== '') $manmodeops.fadeIn(1000);
		else $manmodeops.fadeOut(1000);
	});
	...
	$('#ssfamodal-shortcode-type, #ssfamodal-type').bind('change', function(event){
		...
		if ($sct == "null" || $t == 'null'){
			...
			$('#ssfa-attachaway-table-toggle input:text,\
				...
				#ssfa-fileaway-iframe-toggle input:text,\
				#ssfa-fileaway-iframe-toggle select,\
	    ...
	    if ($sct=="fileaframe"){
			...
			$('#ssfamodal-submit-wrap input:button').removeAttr('disabled').css({cursor : 'pointer'});
			$('#ssfa-fileaway-iframe-toggle input:text,\
			   #ssfa-fileaway-iframe-toggle select').removeAttr('disabled');
		}		
	});
