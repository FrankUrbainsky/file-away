...
<dl class="accordion-secondary">
			...
			<dd style="display:none;">
			File Away is currently developing a secure, front-end file upload framework, that will allow you and specified users to upload files in bulk to the server directories of your choosing, to create directories manually, and/or to auto-generate directories according to specification, such as automated site user directories. In the meantime, the best and simplest solution for creating your file directories and uploading your files to them is an FTP client like <a href="https://filezilla-project.org/" target="_blank">FileZilla</a>. It has drag-and-drop functionality allowing you to upload multiple directories and their contents all at once, from your PC or Mac straight to your server.
			<br />
			<br />
			</dd>
...
<td class="ssfa-minimalist-first-column">[fileaway]</td>
<td>directories</td>
<td>directories=hallelujah (or directories=anything) to enable; leave out to disable</td>
<td>table</td>
...
<td class="ssfa-minimalist-first-column">[fileaway]</td>
<td>playback</td>
<td>playback="longtimecoming" (or playback="anything")</td>
<td>table</td>
<td class="ssfa-minimalist-last-column">For use with tables, Directory Tree tables, or recursive tables. (Not compatible with Manager Mode.) When playback mode is activated, the file type icon for any music files will be replaced with a play/pause button that will stream the audio track to the browser. The audio player is compatible with mp3, ogg, and wav. If any of those file types are found, the player will be added to the column. Note that if you have multiple types with the same filename, then only one will show in the table, and the other file types will be added to the player as fallbacks for greater cross-browser compatibility. For instance: <br /><br />
